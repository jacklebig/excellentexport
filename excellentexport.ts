...
import {utils, write, AOA2SheetOpts} from 'xlsx';
const XLSX = {
    utils,
    write
};
...
const ExcellentExport = function() {
    ...
    const createDownloadLink = function(anchor, base64data, exporttype, filename) {
        if (window.navigator.msSaveBlob) {
            const blob = b64toBlob(base64data, exporttype, null);
    ...
    const s2ab = function (s:string) : (ArrayBuffer|string) {
    ...
    const convert = function(options, sheets) {
        ...
        sheets.forEach(function(sheetConf, index) {
            ...
            const opts = {sheet: name} as AOA2SheetOpts;
            worksheet = XLSX.utils.aoa_to_sheet(dataArray, opts);
